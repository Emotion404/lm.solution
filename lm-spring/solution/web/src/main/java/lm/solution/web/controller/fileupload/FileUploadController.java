...
public class FileUploadController {
    ...
    public String fileUpload_CommonsMultipartResolver(HttpServletRequest request){
        ...
        if(multipartResolver.isMultipart(request)){
            ...
            while (iter.hasNext()){
                ...
                // 调用保存文件的帮助类进行保存文件，并返回文件的相对路径
                String fileName=Files_Utils_DG.FileUpload_transferTo_spring(request,multipartFile,"/files/upload");
    ...
    public String fileUpload_ajax(
            HttpServletRequest request,
            @RequestParam("file_AjaxFile") MultipartFile multipartFile){
        ...
        if(multipartFile!=null&& multipartFile.length>0){
            ...
            try{
                for (int i=0;i<multipartFile.length;i++){
                    ...
                    // 保存文件
                    String fileName=Files_Utils_DG.FileUpload_transferTo_spring(request,file,"/files/upload");
